...
public class cameraControllerPlatform : MonoBehaviour {
	...
	private GameObject depthTester;
	...
	void Start () {
		...
		depthTester = GameObject.FindGameObjectWithTag ("DepthTest");
		linRend = depthTester.GetComponent<LineRenderer> ();
	}
	...
	void Update () {
		...
		depthTester.transform.LookAt (anchor.transform.position); //depthTest node looks anchor.
	...
	void LateUpdate(){
		...
		if (Mathf.Abs(anchor.transform.position.y)>0f) {
			anchor.transform.position = new Vector3 (player.transform.position.x, player.transform.position.y +(2f-(2f/fixedDistanceFromPlayer)), player.transform.position.z); //while your target jumps
		...
		if (Vector3.Distance (transform.position, anchor.transform.position)>fixedDistanceFromPlayer && needsRepos==false) { //distance check from anchor
			if (Vector3.Distance (transform.position, player.transform.position) > fixedDistanceFromPlayer){
				...
				transform.Translate(Vector3.forward*speedDistance*Time.deltaTime);//follow fast
			}
			if(Vector3.Distance(transform.position,player.transform.position) <= fixedDistanceFromPlayer){
				float speedDistance = Math.Abs (Vector3.Distance (transform.position, player.transform.position) - fixedDistanceFromPlayer);
				transform.Translate(Vector3.forward*speedDistance*Time.deltaTime); //follow normal
			}
		}
		...
		if (Vector3.Distance (transform.position, anchor.transform.position)<fixedDistanceFromPlayer && needsRepos==false) { //distance check from anchor
			if(Vector3.Distance(transform.position,player.transform.position) > fixedDistanceFromPlayer){
				float speedDistance = Math.Abs (Vector3.Distance (transform.position, player.transform.position) - fixedDistanceFromPlayer);
				transform.Translate(Vector3.back*speedDistance*Time.deltaTime); //follow fast
			}
			if(Vector3.Distance(transform.position,player.transform.position) <= fixedDistanceFromPlayer){
				float speedDistance = Math.Abs (Vector3.Distance (transform.position, player.transform.position) - fixedDistanceFromPlayer);
				transform.Translate(Vector3.back*speedDistance*Time.deltaTime); //follow normal
			}
		}
	}
	...
	void FixedUpdate(){
		...
		//line cast
		if (Physics.Linecast (new Vector3(depthTester.transform.position.x,depthTester.transform.position.y-0.2f,depthTester.transform.position.z), player.transform.position, out hitInfo) && focusOnTargetLoss==true) {
		...
		try{
			if (needsRepos == true && ((Input.GetAxis("Mouse X")==0) && (Input.GetAxis("Mouse Y")==0))) {
